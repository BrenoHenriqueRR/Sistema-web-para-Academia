...
function exibircomprovante() {
    ...
    var codigoP = document.getElementById("cod").value;
    let valor = '';

    if (formcad) {
        let formdata = new FormData(formcad);
        for (var valores of formdata.entries()) {
            valor += `${valores[0]}: ${valores[1]}\n`;
            console.log(valores[0] + ': ' + valores[1]);
        }
        addEventListener('submit', function (event) {
            event.preventDefault(); // evitar o refresh na pagina quando houver um submit
            event.target.reset(); // resetar os forms
        });

        if (codigoP == '') {
            alert(valor + '\nTotal = R$ 79,80');
        }else{
            alert(valor + "Codigo Promocional: " + codigoP + '\nTotal = R$ 69,80');
        }

        location.href = '../../index.html';
    }

    // Adiciona a nova div ao final do contêiner
}
